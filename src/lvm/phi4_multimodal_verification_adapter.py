"""Phi-4 Multimodal Instruct verification adapter for the frozen framework."""
...
def build_phi4_multimodal_adapter(
    *,
    model_name: str,
    batch_size: int = 1,
    device_map: str = "auto",
    max_new_tokens: int = 512,
    model_key: str = "phi4_multimodal",
    condition: str = "",
    experiment_id: str = "",
) -> "Phi4MultimodalVerificationAdapter":
    """Factory used by the verification adapter registry."""
    return Phi4MultimodalVerificationAdapter(
...
class Phi4MultimodalVerificationAdapter(BaseVerificationAdapter):
    """
    Phi-4 Multimodal Instruct verification adapter.

    Implements model-specific inference only. Orchestration, resume, and
    output persistence are handled by VerificationRunner. Shared parser and
    frozen A1–A5 prompt semantics are unchanged.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._verifier = Phi4MultimodalVerifier(
            model_name=model_name,
            device_map=device_map,
            dtype=dtype,
    ...
    def verify(self, job: VerificationJob) -> VerificationOutcome:
        """Run Phi-4 Multimodal inference for one verification sample."""
        ...
        try:
            ...
            try:
                parsed = parse_verification_response(
                    raw_response, model_key=self.model_key
                )
