"""Gemma 3 verification adapter for the frozen verification framework."""
...
from src.lvm.gemma_verifier import GemmaVerifier
...
def build_gemma_adapter(
    *,
    ...
    condition: str = "",
    experiment_id: str = "",
) -> "GemmaVerificationAdapter":
    """Factory used by the verification adapter registry."""
    return GemmaVerificationAdapter(
...
class GemmaVerificationAdapter(BaseVerificationAdapter):
    """
    Gemma 3 verification adapter.
    ...
    def __init__(
        ...
        self._verifier = GemmaVerifier(model_name=model_name, device_map=device_map)
    ...
    def verify(self, job: VerificationJob) -> VerificationOutcome:
        """Run Gemma 3 inference for one verification sample."""
