"""MiniCPM-V-4.5 verification adapter for the frozen verification framework."""
...
def build_minicpm_v4_5_adapter(
    *,
    model_name: str,
    batch_size: int = 1,
    device_map: str = "auto",
    max_new_tokens: int = 512,
    model_key: str = "minicpm_v4_5",
    condition: str = "",
    experiment_id: str = "",
) -> "MiniCpmV45VerificationAdapter":
    """Factory used by the verification adapter registry."""
    return MiniCpmV45VerificationAdapter(
...
class MiniCpmV45VerificationAdapter(BaseVerificationAdapter):
    """
    MiniCPM-V-4.5 verification adapter.
    ...
    def __init__(
        ...
        *,
        ...
        self._verifier = MiniCpmV45Verifier(
            model_name=model_name,
            device_map=device_map,
            dtype=dtype,
    ...
    def verify(self, job: VerificationJob) -> VerificationOutcome:
        """Run MiniCPM-V-4.5 inference for one verification sample."""
        ...
        try:
            ...
            record["generation"] = {
                "max_new_tokens": self.max_new_tokens,
                # Protocol target. Official MiniCPM chat() uses `sampling`;
                # we map do_sample=False via kwargs (see verifier docstring).
                "do_sample": False,
                "sampling": True,
