...
# Same 10 unique ablation conditions as run_qwen_ablation_100.py
...
def main() -> None:
    ...
    if len(full_metadata_df) < EXPECTED_PALMS:
        print(
            f"Metadata has only {len(full_metadata_df)} row(s); "
            f"need at least {EXPECTED_PALMS} for smoke test."
        )
        sys.exit(1)

    metadata_df = full_metadata_df.head(EXPECTED_PALMS).copy()
    try:
    ...
    print("Qwen ablation smoke test")
    print(f"  Model path: {MODEL_PATH}")
    print(f"  Palms: {len(metadata_df)} (first {EXPECTED_PALMS} from full metadata)")
    print(f"  Unique conditions: {len(ABLATION_CONDITIONS)}")
    print(f"  Expected inferences: {EXPECTED_COMBINED_ROWS}")
    print()
    ...
    if len(combined_df) != EXPECTED_COMBINED_ROWS:
        print(f"WARNING: expected {EXPECTED_COMBINED_ROWS} combined rows.")
        sys.exit(1)

    if total_parse_errors > 0 or total_inference_errors > 0:
        sys.exit(1)
